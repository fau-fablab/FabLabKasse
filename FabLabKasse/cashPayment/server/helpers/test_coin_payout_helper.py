...
def st_coins(draw):
    ...
    st_strange_denominations_and_duplicates = st.lists(
        st.integers(1, 200), average_size=8
    )
    coin_values = draw(
        st.one_of(st_usual_denominations, st_strange_denominations_and_duplicates)
    )
    # sort descending by coin value, and convert to list
    coin_values = sorted(coin_values, reverse=True)
    coin_list = [
        (value, draw(st.integers(min_value=0, max_value=10))) for value in coin_values
    ]
    return coin_list
...
class CoinPayoutHelperTestcase(unittest.TestCase):
    ...
    @example([(100, 0), (50, 0), (20, 0), (10, 0), (5, 0), (2, 0), (1, 0)], 1, 0.2)
    def test_get_possible_payout(self, coins, requested_fraction, coin_limit_fraction):
        ...
        (
            payout_infinite_coin_number,
            allowed_remaining,
        ) = coin_payout_helper.get_possible_payout(coins, max_number_of_coins=int(1e9))
        ...
        (payout_limited_coin_number, _) = coin_payout_helper.get_possible_payout(
            coins, max_number_of_coins=round(coin_limit_fraction * (num_coins + 20))
        )
        ...
        if requested > payout_infinite_coin_number:
            ...
            self.assertGreaterEqual(
                paid_out, payout_infinite_coin_number - allowed_remaining
            )
        else:

...
class Client(AbstractClient):
    ...
    def from_oerp(cls, client_id, oerp):
        ...
        try:
            data = oerp.read(
                "res.partner", client_id, ["name", "x_pin", "credit", "credit_limit"]
            )
...
class ShoppingBackend(AbstractShoppingBackend):
    ...
    def __init__(self, cfg):
        ...
        assert (
            self.cfg.get("openerp", "anonymous_partner_id", None) is not None
        ), "no 'anonymous_partner_id' configured"
        ...
        self.oerp.login(
            user=cfg.get("openerp", "user"), passwd=cfg.get("openerp", "password")
        )

        self.oerp_jcnt = oerplib.rpc.ConnectorJSONRPCSSL(
            cfg.get("openerp", "server"),
            port=cfg.getint("openerp", "port"),
            version=cfg.get("openerp", "version"),
        )
        self.oerp_jcnt.proxy.web.session.authenticate(
            db=cfg.get("openerp", "database"),
    ...
    def create_order(self):
        ...
        order_data = self.oerp.execute(
            "sale.order", "onchange_partner_id", [], partner_id
        )
        print order_data
        assert (
            "warning" not in order_data
        ), u"failed getting default values for sale.order: {0}".format(
            order_data["warning"]
        )
    ...
    def update_quantity(self, order_line_id, amount):
        oerp = self.oerp
        order_line = oerp.read(
            "sale.order.line", order_line_id, ["product_uom_qty"], context=oerp.context
        )
        if order_line["product_uom_qty"] != amount:
            oerp.write(
                "sale.order.line", order_line_id, {"product_uom_qty": float(amount)}
            )
    ...
    def pay_order(self, method):
        ...
        pay_account_id = oerp.read(
            "account.journal", pay_journal_id, ["default_debit_account_id"]
    ...
    def add_order_line(self, prod_id, qty, comment=None):
        ...
        try:
            ...
            raise ProductNotFound(
                "product disapeared: {0}".format(prod_id)
            )  # most likely
        ...
        order_line_data.update(
            {"order_id": order_id, "product_id": prod_id, "product_uom_qty": float(qty)}
        )
    ...
    def _get_products_from_oerp(self, query):
        ...
        """
        oerp = self.oerp
        product_ids = self.oerp.search(
            "product.product", query + [("sale_ok", "=", True)]
        )
        ...
        c = self.oerp.read(
            "product.category",
            current_category,
            ["name", "parent_id", "property_stock_location"],
            context=oerp.context,
        )
        ...
        try:
            pricelist_id = oerp.read(
                "res.partner",
                self.cfg.getint("openerp", "anonymous_partner_id"),
                ["property_product_pricelist"],
                context=oerp.context,
            )["property_product_pricelist"][0]
        except:
            raise Exception(
                "Could not get pricelist from anonymous_client, "
                + "probably wrong id in config.ini."
            )
        product_prices = self.oerp_jcnt.proxy.web.dataset.call(
            model="product.pricelist",
            method="price_get_multi",
            args=[
                [pricelist_id],
        ...
        if "error" in product_prices:
            raise Exception(
                "Could not get prices. Is the anonymous_partner_id set correctly?"
            )
        ...
        for p in products:
            ...
            if p["property_stock_location"]:
                location = (
                    p["property_stock_location"][1]
                    .replace(self.cfg.get("openerp", "strip_location"), "")
                    .strip()
                )
            else:
            ...
            data = Product(
                prod_id=p["id"],
                name=p["name"],
                price=float_to_decimal(p["lst_price"], 3),
                unit=unit,
                location=location,
                categ_id=None,
            )
    ...
    def list_clients(self):
        client_ids = self.oerp.search(
            "res.partner", [("customer", "=", True), ("x_pin", "!=", False)]
        )

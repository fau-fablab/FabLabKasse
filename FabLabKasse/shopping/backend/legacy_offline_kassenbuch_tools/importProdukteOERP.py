...
def categ_id_to_list_of_names(c_id, oerp, cfg):
    categ = oerp.read(
        "product.category", c_id, ["parent_id", "name"], context=oerp.context
    )

    if categ["parent_id"] is False or categ["parent_id"][0] == cfg.getint(
        "openerp", "base_category_id"
    ):
        return [categ["name"]]
    else:
        return categ_id_to_list_of_names(categ["parent_id"][0], oerp, cfg) + [
            categ["name"]
        ]
...
def importProdukteOERP(data, oerp, cfg):
    ...
    prods = oerp.read(
        "product.product",
        prod_ids,
        ["code", "name", "uom_id", "lst_price", "categ_id", "active", "sale_ok"],
